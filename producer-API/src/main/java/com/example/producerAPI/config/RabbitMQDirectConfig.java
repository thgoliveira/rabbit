...
public class RabbitMQDirectConfig {
    ...
    Binding customDirectJuniorBinding(Queue customDirectJuniorQueue, DirectExchange directExchange) {
        return BindingBuilder.bind(customDirectJuniorQueue).to(directExchange).with(RoutingKey.junior);
    }

    @Bean
    Binding customDirectPlenoBinding(Queue customDirectPlenoQueue, DirectExchange directExchange) {
        return BindingBuilder.bind(customDirectPlenoQueue).to(directExchange).with(RoutingKey.pleno);
    }

    @Bean
    Binding customDirectSeniorBinding(Queue customDirectSeniorQueue, DirectExchange directExchange) {
        return BindingBuilder.bind(customDirectSeniorQueue).to(directExchange).with(RoutingKey.senior);
    }

    @Bean
    Binding customDirectSpecBinding(Queue customDirectSpecQueue, DirectExchange directExchange) {
        return BindingBuilder.bind(customDirectSpecQueue).to(directExchange).with(RoutingKey.spec);
    }
